    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if rollout_config.get("enable_two_stage_rollout", False):
                ...
                print(f"[GRLMTrainer] Validation Two-Stage Enabled: {meta_info}")
            ...
            # Inject Beam Search parameters if enabled for validation (Single Stage)
                ...
                print(f"[GRLMTrainer] Validation Beam Search Enabled (optimized, no repeat): {meta_info}")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if not use_beam_search_train:
                    ...
                    print(f"[GRLMTrainer] Beam search enabled, skipping repeat (optimized path)")
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        if rollout_config.get("use_beam_search", False):
                            ...
                            print(f"[GRLMTrainer] Beam Search Enabled (optimized, no repeat): {gen_batch.meta_info}")
                        ...
                        if rollout_config.get("enable_two_stage_rollout", False):
                            ...
                            print(f"[GRLMTrainer] Two-Stage Rollout Enabled: {gen_batch.meta_info}")
                        ...
                        if use_beam_search_train or is_two_stage_rollout:
                            ...
                            input_len = len(batch)
                            print(f"[GRLMTrainer] Beam/TwoStage: gen_batch_output size={output_len}, batch size={input_len}, n={rollout_n}")
                            ...
                            print(f"[GRLMTrainer] Generated UIDs before expansion: {len(batch.non_tensor_batch['uid'])} unique UIDs")
                            ...
                            if output_len > input_len:
                                ...
                                expand_factor = output_len // input_len
                                print(f"[GRLMTrainer] Expanding batch using repeat: factor={expand_factor}")

                                batch = batch.repeat(repeat_times=expand_factor, interleave=True)
                                print(f"[GRLMTrainer] After expansion: batch size={len(batch)}, UIDs will be repeated {expand_factor}x")
