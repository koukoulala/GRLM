...
def read_raw_input(filepath, exclude_ids=None, max_events=100, max_rows=0):
    ...
    RequestTime, HisCount (plus optional others which are preserved for
    split mode but not included in final output).
    ...
    Returns:
        Tuple of (rows, header):
          - rows: list of dicts with keys:
              'raw_parts': original field values (for split mode),
              'user_id', 'events_text' (processed for LLM),
              'profile_text', 'request_time', 'his_count',
              'raw_events': original events string (for output),
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Step 0: Generate shopping journeys from user events "
    ...
    parser.add_argument(
        "--input_file", type=str,
        default="/cosmos/projects/Recommendations/PartnerData/Pipelines/OneRec/Data/1225_0325/JourneyWithProfile/Step0_UserProfile_500KEnUsHisRandom0408_299K.tsv",
        help="Path to input TSV (must have UserId, ReadableUserEvents, "
             "ShoppingProfile, RequestTime, HisCount)",
    )
    parser.add_argument(
        "--output_dir", type=str,
        default="/cosmos/projects/Recommendations/PartnerData/Pipelines/OneRec/Data/1225_0325/JourneyWithProfile/JourneyWithConversationStarterAndDesc_300K/",
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            ...
            "ShoppingJourney_Step1_Gen_Journey_By_UserEventsAndProfile"
            "_ConversationStarterAndDesc_Prompt.md"),
    ...
    parser.add_argument(
        "--num_workers", type=int, default=50,
...
def main():
    ...
    print("=" * 70)
    print("Step 0: Generate Shopping Journeys (with Profile)")
    print("=" * 70)
