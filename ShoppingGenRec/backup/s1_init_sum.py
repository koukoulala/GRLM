...
def process_batch_on_gpu(
    ...
    """Process data slice on specific GPU (CUDA/ROCm) or CPU fallback."""
    ...
    results = []
    debug_printed = 0  # counter for printing first 2 full outputs
    ...
    for i in tqdm(range(len(data_slice)), desc=f"Rank {rank}"):
        ...
        # Print full LLM output for the first 2 items on this rank
        if debug_printed < 2:
            for res in batch_results:
                if debug_printed < 2:
                    print(f"\n{'='*60}")
                    print(f"[Rank {rank} | Debug #{debug_printed+1}] Item ID: {res['id']}")
...
def main():
    ...
    print(f"PyTorch: {torch.__version__}, CUDA: {torch.cuda.is_available()}, HIP: {torch.version.hip}")
    if torch.cuda.is_available():
    ...
    if num_gpus == 0:
        ...
        num_gpus = 1
    elif available_gpus > 0 and num_gpus > available_gpus:
        print(f"Warning: Requested num_gpus={num_gpus} exceeds available GPUs={available_gpus}, using {available_gpus}")
