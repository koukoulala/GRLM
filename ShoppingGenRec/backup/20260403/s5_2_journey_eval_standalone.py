"""Step 5 Journey: Standalone Evaluate Journey Prediction Tasks (SLM only)
...
Produces two TSV output files:

  1. slm_output.tsv           - Model predictions on event2journey inputs.
  2. slm_output_w_profile.tsv - Model predictions on profile2journey inputs.
...
Pipeline:
  ...
  6. Load TID mapping.
  7. Run journey model on event2journey & profile2journey prompts.
  8. Map ProductTIDs back to GlobalOfferIds via exact + fuzzy matching.
  9. Output two TSV files with statistics.
...
Usage:
    python s5_journey_eval_standalone.py \\
        ...
        --sample_n 500 \\
        --num_journeys 3 --min_products 8

    # Let the model decide how many journeys:
    python s5_journey_eval_standalone.py --auto_journeys --min_products 8 ...
"""
...
def parse_args():
    ...
    parser.add_argument("--profile_gpu_memory_utilization", type=float, default=0.75)
    parser.add_argument("--profile_max_model_len", type=int, default=20000)
    parser.add_argument("--profile_max_tokens", type=int, default=8192)
    ...
    parser.add_argument("--fuzzy_score_threshold", type=float, default=6.0)
    parser.add_argument(
    ...
    # Journey / product count
    parser.add_argument(
        "--num_journeys", type=int, default=3,
        help="Number of journeys to predict per user (ignored if --auto_journeys)",
    )
    parser.add_argument(
        "--min_products", type=int, default=8,
        help="Minimum number of products per journey",
    )
    parser.add_argument(
        "--auto_journeys", action="store_true", default=False,
        help="Let the model decide how many journeys to generate",
    )
...
def main():
    ...
    if True:
        ...
        reverse_mapping, word_to_keys, normalized_key_map, sorted_key_map = \
            create_reverse_mapping(tid2item_id, reorder_pos=args.reorder_tid_pos)
    ...
    num_j = args.num_journeys if not args.auto_journeys else None
    e2j_instr_inputs = []
    for ud in users_with_profile:
        instruction, prompt_line = make_journey_instruction(
            "event2journey", num_j, args.min_products,
        )
    ...
    for ud in users_with_profile:
        instruction, prompt_line = make_journey_instruction(
            "profile2journey", num_j, args.min_products,
        )
    ...
    if args.auto_journeys:
        print(f"  Journey count:                  {'auto':>10s}")
    else:
        print(f"  Journey count:                  {args.num_journeys:>10,}")
    print(f"  Min products per journey:       {args.min_products:>10,}")

    print(f"\n  {'Metric':<40s} {'SLM':>10s} {'SLM+Prof':>10s}")
    print(f"  {'-'*40} {'-'*10} {'-'*10}")
    for label, st in [
    ...
    summary = {
        ...
        "profile_model_path": args.profile_model_path,
        "seed": args.seed,
        "num_journeys": args.num_journeys if not args.auto_journeys else "auto",
    ...
    print(f"\nDone! Evaluated {len(users_with_profile)} users on 2 tasks, "
          f"saved 2 output files to {args.output_dir}")
