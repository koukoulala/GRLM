...
def run_profile_generation(users, prompt_template, token_file, copilot_model,
                           num_workers, max_tokens, checkpoint_dir, chunk_size):
    """Generate shopping profiles via Copilot API (wrapper)."""
    # Import the actual Copilot API function from pre_s1 at runtime
    sys.path.insert(0, os.path.join(SCRIPT_DIR, "preprocess_raw_data"))
    from pre_s1_construct_shopping_profile import (
        run_profile_generation_copilot as _run_copilot,
    )
    return _run_copilot(
        users=users, prompt_template=prompt_template, token_file=token_file,
        copilot_model=copilot_model, num_workers=num_workers,
        max_tokens=max_tokens, checkpoint_dir=checkpoint_dir,
