...
- **Data integrity (non-negotiable)**: Do NOT modify any input data. Journey fields and product OfferIds must be copied exactly as provided in the input.
...
The output is intentionally minimal — only the ranking decision (Rank + OfferId) is needed. All other product and journey fields (Title, Seller, Price, Description, etc.) will be reconstructed from the input data in post-processing. Do NOT include them in the output.
...
<OUTPUT>
{
  ...
  "RankingSummary": {
    ...
    "reasoning": "1-2 sentence summary of the key ranking decisions for this journey"
  }
}
</OUTPUT>
...
- **Only products that pass all gates appear in the output.** Filtered products are silently removed.
- Each product in the output includes only `Rank` (1-based, 1 = best) and `OfferId`. Keep-level products are ranked first, then Demoted-level products.
- All other product metadata (Title, Seller, Price, OriginalQuery) and journey fields (Description, ConversationStarter, WhyAmISeeingThis) are reconstructed from the input in post-processing — do NOT include them in the output.
- `OfferId` values MUST be copied exactly from the input. Do not fabricate or modify OfferIds.
