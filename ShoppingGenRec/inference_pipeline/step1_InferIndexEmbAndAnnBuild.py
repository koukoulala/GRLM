"""
step1_InferIndexEmbAndAnnBuild.py
=================================

Pipeline step 1 — infer product embeddings + build a FAISS ANN index.

Stages
------
1. extract  : read product TSV, build a text per product, write
...
All inference assets live under ``./run_matador_emb`` (relative to this
script). Category names come from ``./res/LLMCatMapping.tsv``
(``<CategoryName>\\t<LLMCatId>``).

Examples
--------
    python step1_InferIndexEmbAndAnnBuild.py \\
        --product_file /cosmos/.../IndexData_en_us.tsv \\
    ...
    python step1_InferIndexEmbAndAnnBuild.py \\
        --product_file /tmp/dummy.tsv \\
        --work_dir     ./data/Index_2026_05_04 \\
        --output_prefix EnUs_Product \\
        --only_index --index_type hnsw

    # debug
    python step1_InferIndexEmbAndAnnBuild.py ... --debug --debug_rows 100 --gpu_ids 0
"""
...
# Inference assets (all under ./run_matador_emb)
...
# Category id -> name mapping (./res/LLMCatMapping.tsv: "<name>\t<id>")
LLM_CAT_MAPPING_PATH = os.path.join(SCRIPT_DIR, "res", "LLMCatMapping.tsv")
...
def build_product_text(parts: List[str],
    ...
    """Concatenate Title, Brand, Seller, Category, Gender, Price into a sentence."""
    def get(idx: int) -> str:
        if 0 <= idx < len(parts):
...
# ============================================================================ #
def extract_product_text(product_file: str,
                         text_file: str,
                         id_file: str,
                         cat_mapping: Dict[str, str],
                         has_header: bool,
                         ...
                         col_llm_cat_id: str,
                         max_rows: int = 0) -> int:
    """Read product TSV; write `id\\ttext` and `id`. Returns valid count.

    Column arguments are header names. The header is required (--no_header is
    not supported here).
    """
    if not has_header:
        raise ValueError(
            "extract_product_text requires --has_header (column-name based)."
        )

    print(f"[extract] reading {product_file}")
    if max_rows > 0:
    ...
    with open(product_file, "r", encoding="utf-8") as fin, \
        ...
        # Resolve column names -> 0-based indices from the header line.
    ...
    print(f"[extract] text file: {text_file}")
    print(f"[extract] id   file: {id_file}")
    return valid
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Step 1: infer product embeddings + build FAISS ANN index",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    # I/O ---------------------------------------------------------------------
    g_io = p.add_argument_group("I/O")
    g_io.add_argument("--product_file", default=DEFAULT_PRODUCT_FILE,
                      help="Input product TSV.")
    g_io.add_argument("--work_dir", default=DEFAULT_WORK_DIR,
                      help="Directory for all intermediate + final artefacts.")
    g_io.add_argument("--output_prefix", default="EnUs_Product",
                      help="Filename prefix used for all output files.")

    # Product TSV columns (resolved from header by name) -------------------
    g_col = p.add_argument_group("Product TSV columns (matched by name in header)")
    ...
    g_col.add_argument("--col_llm_cat_id", default="LLMCatId",
                       help="Column whose value is mapped via LLMCatMapping.tsv.")
...
def main() -> None:
    ...
    print("=" * 80)
    print("  step1: Infer index embeddings + build ANN index")
    print("=" * 80)
    print(f"  product_file:   {args.product_file}")
    print(f"  work_dir:       {args.work_dir}")
    print(f"  output_prefix:  {args.output_prefix}")
    print(f"  inference_dir:  {INFERENCE_DIR}")
    print(f"  cat mapping:    {LLM_CAT_MAPPING_PATH}")
    ...
    if not args.skip_extract:
        print("-" * 80)
        print("Stage 1/4: extract product text")
        print("-" * 80)
        cat_mapping = load_llm_cat_mapping(LLM_CAT_MAPPING_PATH)
        n = extract_product_text(
